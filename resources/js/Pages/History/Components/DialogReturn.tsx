...
export default function DialogReturn({ open, onClose, values }: DialogAddProps) {
     ...
     const handleSubmit = (e: any) => {
          ...
          post(route('rents.update', values.id), {
               onError: (errors) => {
                    console.log('errors', errors);
                    setIsLoading(false)
                    toast.error(errors.message || 'Failed to return car, please check your data')
               },
               onSuccess: (msg) => {
                    console.log(msg);
                    setIsLoading(false)
                    toast.success('Success return car')
                    handleClose()
               }
          })
          setIsLoading(false)
     ...
     useEffect(() => {
          if (values && open) {
               let totalPrice = values?.car?.price * ((moment().diff(moment(values.start_date), 'days')) || 1)
               if (totalPrice < 0) {
                    totalPrice = 0
               }
               setData({
                         ...
                         <Grid container spacing={1} className="w-full ">
                              <Grid item xs={12} >
                                   <div className='flex sm:flex-row flex-col gap-3 p-3 border border-orange-500 rounded-lg shadow mb-20'>
                                        <img src={`storage/${values?.car?.image}`} alt={values?.car?.brand} className='sm:h-28 object-cover rounded-lg' />
                                        <div className='flex flex-col justify-between'>
                                             <div>
                                                  <Typography variant='h6' sx={{ fontWeight: 600, color: 'rgba(76, 78, 100, 0.87)' }}>{values?.car?.brand} </Typography>
                                                  <Typography variant='body2' sx={{ color: 'rgba(76, 78, 100, 0.87)' }}>{values?.car?.model}</Typography>
                                             </div>
                                             <Typography variant='body2' fontWeight={'bold'} sx={{ color: 'rgba(76, 78, 100, 0.87)' }}>{values?.car?.plate_number}</Typography>
                                        </div>
                                        <p className='text-3xl font-bold text-orange-500 ms-auto'>{formatPrice(values?.car?.price || 0)}
                                             <span
